...
constexpr spi_mode_t kSpiMode = SPI_MODE0;
} 
...
lr1121_err lr1121::init() {
  ...
  if (gpio_config(&io) != ESP_OK) return lr1121_err::invalid_arg;
  ...
  if (err != ESP_OK) return lr1121_err::spi_transfer;
...
lr1121_err lr1121::write_cmd(uint8_t op_hi, uint8_t op_lo) {
  ...
  return (err == ESP_OK) ? lr1121_err::OK : lr1121_err::spi_transfer;
}

lr1121_err lr1121::read_response(uint8_t* rx, size_t len) {
  if (!rx || len == 0) return lr1121_err::invalid_arg;
...
lr1121_err lr1121::get_version(lr1121_version* out) {
  if (!out) return lr1121_err::invalid_arg;
