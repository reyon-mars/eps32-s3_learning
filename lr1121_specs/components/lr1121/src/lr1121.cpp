...
Lr1121Err Lr1121::init() {
  ...
  if (gpio_config(&io) != ESP_OK) return Lr1121Err::InvalidArg;
  ...
  if (err != ESP_OK) return Lr1121Err::SpiTransfer;
...
Lr1121Err Lr1121::read_response(uint8_t* rx, size_t len) {
  if (!rx || len == 0) return Lr1121Err::InvalidArg;
...
Lr1121Err Lr1121::get_version(Lr1121Version* out) {
  if (!out) return Lr1121Err::InvalidArg;
